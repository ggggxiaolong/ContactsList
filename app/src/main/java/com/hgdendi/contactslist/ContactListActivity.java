...
public class ContactListActivity extends AppCompatActivity {
  ...
  private void initView() {
    mRecyclerView = findViewById(R.id.share_add_contact_recyclerview);
  ...
  private void showLetterHintDialog(String s) {
    if (mOperationInfoDialog == null) {
      ...
      mOperationInfoDialog = new PopupWindow(mLetterHintView, ActionBar.LayoutParams.WRAP_CONTENT,
    ...
    getWindow().getDecorView().post(new Runnable() {
      @Override
      public void run() {
        mOperationInfoDialog.showAtLocation(
            getWindow().getDecorView().findViewById(android.R.id.content), Gravity.CENTER, 0, 0);
      }
    });
  }
  ...
  protected void fetchContactList() {
    if (Utils.checkHasPermission(this, Manifest.permission.READ_CONTACTS,
  ...
  private void preOperation() {
    ...
    for (int i = 1; i < mContactList.size(); i++) {
      if (!mContactList.get(i - 1)
          .getInitial()
          .equalsIgnoreCase(mContactList.get(i).getInitial())) {
